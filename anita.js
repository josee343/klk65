...
var prefix = '.' //prefijo
var NombreBot = '1eroA Bot' // nombre del bot 
var Creador = "Josè Anibal Santana" // No cambiar
...
anita.sendMessage(id, buttonMessage, {quoted: vr}) }
//*******************************************//
const enviartexto = (texto) => {anita.sendMessage(from,{text: texto }, {quoted : doc})
}
const enviarimagen = (imagen) => {anita.sendMessage(from,{image: imagen }, {quoted : doc})
}
const enviarimagencap = (imagen,caption) => {anita.sendMessage(from,{image: imagen ,caption }, {quoted : doc})
}
const enviarvideos = (videos) => {anita.sendMessage(from,{ video: videos }, {quoted : doc})
}
const enviarvideoscap = (videos,caption) => {anita.sendMessage(from,{video: videos ,caption }, {quoted : doc})
}
const enviarmusica = (Audios) => {anita.sendMessage(from,{Audio: Audios  }, {quoted : doc})
}
const enviarsticker = (Sticker) => {anita.sendMessage(from,{Sticker: sticker  }, {quoted : doc})
}
...
switch(command){
  ...
  case 'Saludos' :
  enviartexto('Hola a todos. Es un honor estar aquí y agradecerle por usar 1eroA bot.')
  break

  case 'Proyectos' :
  enviartexto('No hay proyectos asignado de un Maestro.')
  break

  case 'Calendario' :
  const agenda = fs.readFileSync('./archivos/fotos/Calendario.jpg')
  const dws = 'Calendario de 1eroA de Secundaria-2023'
  enviarimagencap(agenda,dws)
  break

  case 'Obras' :
  const Lectura = fs.readFileSync('./archivos/fotos/Obras.jpg')
  const dwa = 'Listado de las obras de 1eroA de Secundaria-2023'
  enviarimagencap(Lectura,ddwa)
  break

  case 'Creditos' :
  enviartexto('El creador de Bot es Josè Anibal Santana Herrera (Presidente de Curso)')
  break
...
if(!isGroup) return enviartexto ('Este comando solo es para grupos')

const gotu365 = fs.readFileSync('./archivos/fotos/bot.jpg')
